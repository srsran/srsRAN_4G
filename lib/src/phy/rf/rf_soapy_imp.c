...
{
    rf_soapy_handler_t *handler = (rf_soapy_handler_t*) h;
 SoapySDRRange range = SoapySDRDevice_getGainRange(handler->device, SOAPY_SDR_RX,0);
...
{
    rf_soapy_handler_t *handler = (rf_soapy_handler_t*) h;
 SoapySDRRange range = SoapySDRDevice_getGainRange(handler->device, SOAPY_SDR_TX,0);
...
double rf_soapy_set_tx_gain(void *h, double gain)
{
    ...
    double ret = rf_soapy_get_tx_gain(h);
    printf("gain has been set to %f \n",ret);
    return ret;
}
...
double rf_soapy_set_tx_freq(void *h, double freq)
{
    ...
    double ret = SoapySDRDevice_getFrequency(handler->device, SOAPY_SDR_TX, 0);
    printf("Frequency has been set to %f : \n",ret);
    return ret;
