...
int32 arrcmp(uint8_t const * const a, uint8_t const * const b,
		uint32 len) {
	int32 i = 0;
...
START_TEST(test_set_1)
	{
		...
		ck_assert_int_eq(err_lte, LIBLTE_SUCCESS);

		// compare cipher text
		err_cmp = arrcmp(ct, out, len_bytes);
		ck_assert_int_eq(err_cmp, 0);
...
START_TEST(test_set_1_block_size)
	{
		LIBLTE_ERROR_ENUM err_lte = LIBLTE_ERROR_INVALID_INPUTS;
		int32 err_cmp = 0;
...
int main(int argc, char * argv[]) {
	...
	// test suite
	suite = suite_create("eea2");

	// test case
	tcase = tcase_create("encryption_and_decryption");
	tcase_add_test(tcase, test_set_1);
	tcase_add_test(tcase, test_set_2);
	tcase_add_test(tcase, test_set_3);
	tcase_add_test(tcase, test_set_4);
	tcase_add_test(tcase, test_set_5);
	tcase_add_test(tcase, test_set_6);
	tcase_add_test(tcase, test_set_1_block_size);
	tcase_add_test(tcase, test_set_1_invalid);
