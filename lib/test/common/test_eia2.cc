 ...
 * Tests
 *
 * Document Reference: 33.401 V15.6.0 Annex C.2
 *
 * Only byte-aligned test sets are exercised because the srsran::security_128_eia2()
 * wrapper takes the message length in bytes (the underlying implementation operates
 * on whole-byte input). Sets 1, 3, 4, 6 and 7 from Annex C.2 have non-octet-aligned
 * message lengths and are therefore not covered here.
 */
...
int test_set_2()
{
  ...
  uint8_t  bearer    = 0x1a;
  uint8_t  direction = 1;
  uint32_t len_bits = 64, len_bytes = (len_bits + 7) / 8;
  uint8_t  msg[] = {0x48, 0x45, 0x83, 0xd5, 0xaf, 0xe0, 0x82, 0xae};
  uint8_t  mt[]  = {0xb9, 0x37, 0x87, 0xe6};

  uint8_t mac[4];

  // gen mac
  srsran::security_128_eia2(key, count, bearer, direction, msg, len_bytes, mac);
...
int test_set_5()
{
  ...
  uint8_t  mt[]  = {0xe6, 0x57, 0xe1, 0x82};
...
int test_set_8()
{
  ...
  uint8_t mt[] = {0xeb, 0xd5, 0xcc, 0xb0};
...
int main(int argc, char* argv[])
{
  ...
  TESTASSERT(test_set_8() == SRSRAN_SUCCESS);
  return SRSRAN_SUCCESS;
}
