...
void phch_worker::init(phch_common* phy_, srslte::log *log_h_)
{
  ...
  if (srslte_faux_enb_dl_init(&enb_dl, phy->cell.nof_prb)) {
    fprintf(stderr, "Error initiating ENB DL\n");
    return;
  }
  if (srslte_faux_enb_dl_set_cell(&enb_dl, phy->cell)) {
    fprintf(stderr, "Error initiating ENB DL\n");
    return;
  }
  if (srslte_faux_enb_ul_init(&enb_ul, phy->cell.nof_prb)) {
    fprintf(stderr, "Error initiating ENB UL\n");
    return;
  }
  if (srslte_faux_enb_ul_set_cell(&enb_ul,
                              phy->cell,
                              NULL,
                              &phy->pusch_cfg,
                              &phy->hopping_cfg,
  ...
  srslte_faux_enb_dl_set_amp(&enb_dl, phy->params.tx_amplitude);
...
void phch_worker::stop()
{
  ...
  srslte_faux_enb_dl_free(&enb_dl);
  srslte_faux_enb_ul_free(&enb_ul);
  if (signal_buffer_rx) {
...
{
  X_TRACE("PHCHWORKER: rnti %hu", rnti);

  if (srslte_faux_enb_dl_add_rnti(&enb_dl, rnti)) {
    return -1; 
  }
  if (srslte_faux_enb_ul_add_rnti(&enb_ul, rnti)) {
    return -1; 
  }
...
uint32_t phch_worker::get_nof_rnti() {
  X_TRACE("PHCHWORKER: %zu", ue_db.size());
                                       ...
                                       uint32_t I_sr, bool pucch_cqi, uint32_t pmi_idx, bool pucch_cqi_ack)
{
  ...
  if (ue_db.count(rnti)) {
    ...
    srslte_faux_enb_ul_cfg_ue(&enb_ul, rnti, uci_cfg, pucch_sched, srs_cfg);
...
{
  X_TRACE("PHCHWORKER: %hu", rnti);

  pthread_mutex_lock(&mutex); 
  if (ue_db.count(rnti)) {
    ue_db.erase(rnti);
    
    srslte_faux_enb_dl_rem_rnti(&enb_dl, rnti); 
    srslte_faux_enb_ul_rem_rnti(&enb_ul, rnti);
...
void phch_worker::work_imp()
{
  ...
  mac_interface_faux_phy::ul_sched_t *ul_grants = phy->ul_grants;
  mac_interface_faux_phy::dl_sched_t *dl_grants = phy->dl_grants; 
  mac_interface_faux_phy *mac = phy->mac; 
  
  log_h->step(tti_rx);
  
  Debug("Worker %d running tti_rx/tx %u/%u, sf_rx/tx %u/%u\n", 
  ...
  srslte_faux_enb_dl_clear_sf(&enb_dl);
  srslte_faux_enb_dl_set_cfi(&enb_dl, dl_grants[sf_tx].cfi);
...
{
  X_TRACE("PHCHWORKER: nof_pusch %u, tti %u", nof_pusch, tti);
  ...
  for (uint32_t i=0;i<nof_pusch;i++) {
    ...
    if (rnti) {
      ...
      if (!srslte_ra_ul_dci_to_grant(&grants[i].grant, enb_ul.cell.nof_prb, n_rb_ho, &phy_grant, tti%8)) {
        ...
        res = srslte_faux_enb_ul_get_pusch(&enb_ul, &phy_grant, grants[i].softbuffer,
...
{
  X_TRACE("PHCHWORKER: tti_rx %u", tti_rx);
  ...
  for(std::map<uint16_t, ue>::iterator iter=ue_db.begin(); iter!=ue_db.end(); ++iter) {
    ...
    if (rnti >= SRSLTE_CRNTI_START && rnti <= SRSLTE_CRNTI_END && ue_db[rnti].has_grant_tti != (int) tti_rx) {
      ...
      if (needs_pucch) {
        if (srslte_faux_enb_ul_get_pucch(&enb_ul, rnti, last_n_pdcch, sf_rx, &uci_data)) {
...
{
  X_TRACE("PHCHWORKER: acks %u, sf_idx %u", nof_acks, sf_idx);

  for (uint32_t i=0;i<nof_acks;i++) {
    uint16_t rnti = acks[i].rnti;
    if (rnti) {
      srslte_faux_enb_dl_put_phich(&enb_dl, acks[i].ack, 
...
{
  X_TRACE("PHCHWORKER: grants %u, sf_idx %u", nof_grants, sf_idx);

  for (uint32_t i=0;i<nof_grants;i++) {
    uint16_t rnti = grants[i].rnti;
    if (grants[i].needs_pdcch && rnti) {
      if (srslte_faux_enb_dl_put_pdcch_ul(&enb_dl, &grants[i].grant, grants[i].location, rnti, sf_idx)) {
...
int phch_worker::encode_faux_pdcch_dl(srslte_faux_enb_dl_pdsch_t *grants, uint32_t nof_grants, uint32_t sf_idx)
{
  ...
  for (uint32_t i=0;i<nof_grants;i++) {
    ...
    if (rnti) {
      ...
      if (srslte_faux_enb_dl_put_pdcch_dl(&enb_dl, &grants[i].grant, format, grants[i].location, rnti, sf_idx)) {
...
int phch_worker::encode_faux_pdsch(srslte_faux_enb_dl_pdsch_t *grants, uint32_t nof_grants, uint32_t sf_idx)
{
  ...
  for (uint32_t i=0;i<nof_grants;i++) {
    ...
    if (rnti) {
      ...
      if (srslte_faux_enb_dl_put_pdsch(&enb_dl, &phy_grant, sb, rnti, rv, sf_idx, d, SRSLTE_MIMO_TYPE_SINGLE_ANTENNA, 0))
      {
