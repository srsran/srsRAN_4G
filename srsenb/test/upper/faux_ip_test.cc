...
void usage(prog_args_t *args, char *prog) {
  Debug("Usage: %s [gGIrfFdv] \n", prog);
  Debug("\t-f RX frequency [Default %.1f MHz]\n", args->rx_freq/1e6);
  Debug("\t-F TX frequency [Default %.1f MHz]\n", args->tx_freq/1e6);
  Debug("\t-g RX gain [Default %.1f]\n", args->rx_gain);
  Debug("\t-G TX gain [Default %.1f]\n", args->tx_gain);
  Debug("\t-I IP address [Default %s]\n", args->ip_address.c_str());
  Debug("\t-t time advance (in samples) [Default %d]\n", args->time_adv);
  Debug("\t-d Enable gui [Default disabled]\n");
  Debug("\t-v [increase verbosity, default none]\n");
}
...
class faux_phy_tester : public srsue::pdcp_interface_rlc, 
...
public:
  ...
  void write_pdu(uint32_t lcid, srslte::byte_buffer_t *sdu)
  {
    ...
    if (n != (int) sdu->N_bytes) {
      Error("TUN/TAP write failure n=%d, nof_bytes=%d\n", n, sdu->N_bytes);
      return; 
    }

    log_h->debug_hex(sdu->msg, sdu->N_bytes, "Wrote %d bytes to TUN/TAP\n", sdu->N_bytes);         
  ...
  void write_pdu(uint16_t rnti, uint32_t lcid, uint8_t *payload, uint32_t nof_bytes)
  {
    ...
    switch(lcid) {
      ...
      break;
      case 0:        
        Info("Received ConnectionRequest from rnti=0x%x\n", rnti);
        ...
        faux_phy->set_config_dedicated(rnti, &dedicated);
        
        usleep(500);
      break;
      default:
        Error("Received message for lcid=%d\n", lcid);
      break;
    }    
  }
...
private:
  ...
  int init_tuntap(char *ip_address) {
    ...
    Debug("Created tun/tap interface at IP %s\n", ip_address);
    return 0; 
  }
  ...
  void run_thread() {
    ...
    Info("TUN/TAP reader thread running\n");
    ...
    while(running) {
      ...
      if(N_bytes > 0)
      {
        ...
        Error("Failed to read from TUN interface - gw receive thread exiting.\n");
        break;
      }
    }
  }  
