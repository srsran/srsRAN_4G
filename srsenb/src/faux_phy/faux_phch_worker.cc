...
#include "faux_phy/faux_phch_worker.h"
...
void phch_worker::init(phch_common* phy_, srslte::log *log_h_)
{
  ...
  Info("Worker %d configured cell %d PRB", get_id(), phy->cell.nof_prb);
                                       ...
                                       uint32_t I_sr, bool pucch_cqi, uint32_t pmi_idx, bool pucch_cqi_ack)
{
  ...
  if (ue_db.count(rnti)) {
    ...
    Error("Setting config dedicated: rnti=0x%x does not exist", rnti);
  }
  pthread_mutex_unlock(&mutex); 
}
...
void phch_worker::rem_rnti(uint16_t rnti)
{
  ...
  if (ue_db.count(rnti)) {
    ...
    Error("Removing user: rnti=0x%x does not exist", rnti);
  }
  pthread_mutex_unlock(&mutex); 
}
...
void phch_worker::work_imp()
{
  ...
  if (dl_grants[sf_tx].cfi < 1 || dl_grants[sf_tx].cfi > 3) {
    Error("Invalid CFI=%d", dl_grants[sf_tx].cfi);
    goto unlock;
  }
      ...
      */
      ...
      if (grants[i].grant.rv_idx == 0) {
        if (res && snr_db < PUSCH_RL_SNR_DB_TH) {
          Debug("PUSCH: Radio-Link failure snr=%.1f dB", snr_db);
  ...
  for(std::map<uint16_t, ue>::iterator iter=ue_db.begin(); iter!=ue_db.end(); ++iter) {
    ...
    if (rnti >= SRSLTE_CRNTI_START && rnti <= SRSLTE_CRNTI_END && ue_db[rnti].has_grant_tti != (int) tti_rx) {
      ...
      if (needs_pucch) {
        ...
        Info("PUCCH: rnti=0x%x, corr=%.2f, n_pucch=%d, n_prb=%d%s%s%s", 
                    rnti, 
        ...
        if (!needs_sr) {
          if (srslte_pucch_get_last_corr(&enb_ul.pucch) < PUCCH_RL_CORR_TH) {
            Debug("PUCCH: Radio-Link failure corr=%.1f", srslte_pucch_get_last_corr(&enb_ul.pucch));
            phy->mac->rl_failure(rnti);
          } else {
            phy->mac->rl_ok(rnti);
          }          
  ...
  for (uint32_t i=0;i<nof_grants;i++) {
    ...
    if (grants[i].needs_pdcch && rnti) {
      if (srslte_enb_dl_put_pdcch_ul(&enb_dl, &grants[i].grant, grants[i].location, rnti, sf_idx)) {
        fprintf(stderr, "Error putting PUSCH %d",i);
        return SRSLTE_ERROR; 
      }
  ...
  for (uint32_t i=0;i<nof_grants;i++) {
    ...
    if (rnti) {
      ...
      if (srslte_enb_dl_put_pdcch_dl(&enb_dl, &grants[i].grant, format, grants[i].location, rnti, sf_idx)) {
        fprintf(stderr, "Error putting PDCCH %d",i);
        return SRSLTE_ERROR; 
      }      
      
      if (LOG_THIS(rnti)) {
        Info("PDCCH: DL DCI %s rnti=0x%x, cce_index=%d, L=%d, tti_tx=%d", srslte_dci_format_string(format), 
          rnti, grants[i].location.ncce, (1<<grants[i].location.L), tti_tx);
      }
    }
  }
...
int phch_worker::encode_pdsch(srslte_enb_dl_pdsch_t *grants, uint32_t nof_grants, uint32_t sf_idx)
{
  for (uint32_t i=0;i<nof_grants;i++) {
    ...
    if (rnti) {
      ...
      {
        fprintf(stderr, "Error putting PDSCH %d",i);
        return SRSLTE_ERROR; 
      }
...
void phch_worker::start_plot() {
  ...
  if (plot_worker_id == -1) {
    ...
    log_h->console("Starting plot for worker_id=%d", plot_worker_id);
    init_plots(this);
  } else {
    log_h->console("Trying to start a plot but already started by worker_id=%d", plot_worker_id);
  }
#else 
