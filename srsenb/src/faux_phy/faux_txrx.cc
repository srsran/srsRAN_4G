...
void txrx::run_thread()
{
  ...
  Debug("Setting Sampling frequency %.2f MHz", (float) samp_rate/1000000);

  // Configure radio 
  radio_h->set_rx_srate(samp_rate);
  radio_h->set_tx_srate(samp_rate);  
  
  Debug("Starting RX/TX thread nof_prb=%d, sf_len=%d",worker_com->cell.nof_prb, sf_len);
