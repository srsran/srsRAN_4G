...
#include "faux_phy/faux_prach_worker.h"
  ...
  if (srslte_prach_tti_opportunity(&prach, tti_rx, -1) || sf_cnt) {
    ...
    if (sf_cnt == nof_sf) {
      ...
      if ((int) pending_tti != processed_tti) {
        Warning("PRACH thread did not finish processing TTI=%d", pending_tti);
      }
      pthread_mutex_lock(&mutex);
      if (tti_rx+1 > nof_sf) {
...
int prach_worker::run_tti(uint32_t tti_rx)
{
  if (srslte_prach_tti_opportunity(&prach, tti_rx, -1)) 
  {
    ...
    if (srslte_prach_detect_offset(&prach,
                                   ...
                                   &prach_nof_det)) 
    {
      Error("Error detecting PRACH");
      return SRSLTE_ERROR; 
    }
    
    if (prach_nof_det) {
      for (uint32_t i=0;i<prach_nof_det;i++) {
        Info("PRACH: %d/%d, preamble=%d, offset=%.1f us, peak2avg=%.1f, max_offset=%.1f us", 
...
void prach_worker::run_thread()
{
  ...
  while(running) {
   ...
   pthread_mutex_unlock(&mutex);
   Debug("Processing pending_tti=%d", pending_tti);
   if (running) {
