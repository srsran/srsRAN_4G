...
{
public:
  paging_manager(uint32_t default_paging_cycle_, float nb_, bool etws_present, bool cmas_present) :
    ...
    logger(srslog::fetch_basic_logger("RRC"))
  {
    for (subframe_info& sf_obj : sf_pending_pcch) {
      ...
      for (pcch_info& pcch : sf_obj.pending_paging) {
        ...
        pcch.pcch_msg.msg.set_c1().paging().etws_ind_present = etws_present;
        if (cmas_present) {
          pcch.pcch_msg.msg.set_c1().paging().non_crit_ext_present                          = true;
          pcch.pcch_msg.msg.set_c1().paging().non_crit_ext.non_crit_ext_present             = true;
          pcch.pcch_msg.msg.set_c1().paging().non_crit_ext.non_crit_ext.cmas_ind_r9_present = true;
        }
      }
    }
  }
